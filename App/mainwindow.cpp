...
MainWindow::MainWindow(QWidget *parent)
  ...
  connect(_roomEventAdapter.get(), &RoomEventAdapter::roomStateChanged, this,
          &MainWindow::onRoomStateChanged);

  _mediaEventAdapter = std::make_shared<MediaEventAdapter>(this);
  connect(_mediaEventAdapter.get(), &MediaEventAdapter::videoTrackCreated, this,
          &MainWindow::onVideoTrackCreated);
  connect(_mediaEventAdapter.get(), &MediaEventAdapter::videoTrackRemoved, this,
          &MainWindow::onVideoTrackRemoved);

  _participantEventAdapter = std::make_shared<ParticipantEventAdapter>(this);
  connect(_participantEventAdapter.get(),
          &ParticipantEventAdapter::participantCreated, this,
          &MainWindow::onParticipantCreated);
  connect(_participantEventAdapter.get(),
          &ParticipantEventAdapter::participantUpdated, this,
          &MainWindow::onParticipantUpdated);
  connect(_participantEventAdapter.get(),
          &ParticipantEventAdapter::participantRemoved, this,
          &MainWindow::onParticipantRemoved);
}
...
void MainWindow::onVideoTrackCreated(const std::string &id,
  ...
  std::shared_ptr<ContentView> view =
      std::make_shared<ContentView>(id, vt, renderer);
  view->init();

  _galleryView->insertView(view);
}

void MainWindow::onVideoTrackRemoved(const std::string &id,
                                     webrtc::MediaStreamTrackInterface *track) {
  if (!track) {
    return;
  }
...
void MainWindow::onParticipantCreated(
    std::shared_ptr<vi::IParticipant> participant) {}

void MainWindow::onParticipantUpdated(
    std::shared_ptr<vi::IParticipant> participant) {}

void MainWindow::onParticipantRemoved(
    std::shared_ptr<vi::IParticipant> participant) {}

void MainWindow::closeEvent(QCloseEvent *event) { destroy(); }
