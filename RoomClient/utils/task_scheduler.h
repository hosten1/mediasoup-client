...
namespace vi
{
    ...
    class OneShotTask : public rtc::TaskQueue
	{
	public:
	...
	template <class Closure>
	class RepetitiveTask : public rtc::TaskQueue
	{
	public:
	...
	private:
		...
		std::unique_ptr<rtc::TaskQueue> createOneShotTask(Closure &&closure, uint64_t id)
		{
			return std::make_unique<OneShotTask<Closure>>(std::forward<Closure>(closure), id, weak_from_this());
		}

		template <typename Closure>
		std::unique_ptr<rtc::TaskQueue> createRepetitiveTask(Closure &&closure, uint32_t milliseconds, uint64_t id)
		{
