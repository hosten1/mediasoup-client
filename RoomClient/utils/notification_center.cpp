...
namespace {
static std::shared_ptr<vi::NotificationCenter> _defaultCenter =
    std::make_shared<vi::NotificationCenter>();
}
...
void NotificationCenter::postNotification(
    const std::shared_ptr<INotification> &notification) {
...
void NotificationCenter::removeInvalidObservers() {
  _observers.erase(
      std::remove_if(_observers.begin(), _observers.end(),
                     [](const auto &observer) { return !observer->isValid(); }),
      _observers.end());
}
  ...
  for (const auto &observer : observers) {
    if (observer && observer->shouldAccept(notification)) {
      ...
      if (thread->IsCurrent()) {
        ...
        thread->PostTask(
            RTC_FROM_HERE,
            [obs = std::weak_ptr<IObserver>(observer), notification]() {
