...
/// 4. The observer can only receive the type of notification it subscribes to
/// (The observer subscribes the 'KeyValueNotification' will not accept the
/// other types of notifications, better performance)
...
/// {
/// public:
///     void onKeyValueChanged(const std::shared_ptr<KeyValueNotification>&
///     kvnf) {} void onCustomNotification(const
///     std::shared_ptr<CustomNotification>& cnf) {}
/// };
///
...
///    nfc->addObserver(Observer<MyClass, KeyValueNotification>(myClass,
///    &MyClass::onKeyValueChanged, ThreadName::WORKER));
///    nfc->removeObserver(Observer<MyClass, KeyValueNotification>(myClass,
///    &MyClass::onKeyValueChanged));
/// }
///
/// 3. Post Key-Value Notification:
///
/// if (auto nfc = NotificationCenter::defaultCenter()) {
///     auto nf = std::make_shared<KeyValueNotification>("jackie's key",
///     std::string("hello")); nfc->postNotification(nf);
/// }
///
/// 4. Define a custom notification:
///
/// class CustomNotification : public INotification, public
/// std::enable_shared_from_this<CustomNotification>
/// {
/// public:
///     CustomNotification(int32_t value1, int32_t value2) : m_value1(value1),
///     m_value2(value2) { } int32_t value1() const { return m_value1; } int32_t
///     value2() const { return m_value2; }
/// private:
///     int32_t m_value1;
///     int32_t m_value2;
/// };
...
/// auto myClass = std::make_shared<MyClass>();
/// if (auto nfc = NotificationCenter::defaultCenter()) {
///    nfc->addObserver(Observer<MyClass, CustomNotification>(myClass,
///    &MyClass::onCustomNotification));
/// }
///
