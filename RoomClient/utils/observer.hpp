...
#ifndef ROOMCLIENT_UTILS_OBSERVER_H_
#define ROOMCLIENT_UTILS_OBSERVER_H_
...
namespace vi
{
    ...
    public:
        ...
        Observer(const std::shared_ptr<T> &object, Method method)
            : _object(object), _method(method), _scheduleThread(std::string("main"))
        {
        }
        ...
        Observer(const Observer &observer)
            : _object(observer._object), _method(observer._method), _scheduleThread(observer._scheduleThread)
        {
        }
...
#endif // !ROOMCLIENT_UTILS_OBSERVER_H_
