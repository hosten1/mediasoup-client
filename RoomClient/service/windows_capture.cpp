...
namespace vi
{
	...
	VcmCapturer::VcmCapturer()
		: vcm_(nullptr), thread_(TMgr->thread("capture-session"))
	{
	}
						   ...
						   size_t capture_device_index)
	{
		...
		vcm_ = thread_->BlockingCall([&]
									 { return VideoCaptureFactory::Create(unique_name); });
		if (!vcm_)
		{
			return false;
		}
		...
		int32_t code = thread_->BlockingCall([&]
											 { return vcm_->_startCapture(capability_); });
		if (code != 0)
		{
			Destroy();
			return false;
	...
	void VcmCapturer::Destroy()
	{
		...
		thread_->BlockingCall([&]
							  { return _stopCapture(this); });

		vcm_->DeRegisterCaptureDataCallback();
		DLOG("destroy capture source3");
