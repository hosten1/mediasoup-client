...
void SignalingClient::addObserver(
    std::shared_ptr<ISignalingObserver> observer) {
  UniversalObservable<ISignalingObserver>::addWeakObserver(
      observer, "signaling-transport");
}
...
void SignalingClient::onOpened() {
  UniversalObservable<ISignalingObserver>::notifyObservers(
...
void SignalingClient::handleResponse(const std::string &json) {
  if (auto thread = TMgr->thread("signaling-transport")) {
    TMgr->thread("signaling-transport")
        ->PostTask([wself = weak_from_this(), json]() {
