...
RoomClient::RoomClient(std::weak_ptr<IComponentFactory> wcf) : _wcf(wcf) {}
...
void RoomClient::init() {
  ...
  if (!_mediaController.lock()) {
    ...
    _mediaControllerProxy =
        MediaControllerProxy::create(mediaController, "mediasoup-client");
  ...
  if (!_participantController.lock()) {
    ...
    _participantControllerProxy = ParticipantControllerProxy::create(
...
void RoomClient::addObserver(std::shared_ptr<IRoomClientObserver> observer) {
  UniversalObservable<IRoomClientObserver>::addWeakObserver(observer,
                                                            "mediasoup-client");
}
  ...
  _mediasoupApi->createWebRtcTransport(
      ...
      [wself = weak_from_this(), consuming, producing](
        ...
        TMgr->thread("mediasoup-client")
            ->PostTask([wself, producing, consuming, response]() {
              auto self = wself.lock();
              if (!self) {
                DLOG("RoomClient is null");
                return;
...
void _onConnect(mediasoupclient::Transport *transport,
  ...
  _mediasoupApi->connectWebRtcTransport(
      request, [wself = weak_from_this(),
                &promise](int32_t errorCode, const std::string &errorInfo,
                          std::shared_ptr<signaling::BasicResponse> response) {
        auto self = wself.lock();
        if (!self) {
          DLOG("RoomClient is null");
