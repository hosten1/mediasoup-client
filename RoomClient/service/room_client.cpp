...
namespace vi
{
    class RoomClientImpl : public ISignalingObserver,
                           ...
                           public std::enable_shared_from_this<RoomClientImpl>
    {
    public:
        RoomClientImpl(std::weak_ptr<IComponentFactory> wcf)
            : _wcf(wcf),
              _inited(false),
              _destroy(false)
        {
        ...
        void addObserver(std::shared_ptr<IRoomClientObserver> observer)
        {
            UniversalObservable<IRoomClientObserver>::addWeakObserver(observer, "mediasoup-client");
        }

        void removeObserver(std::shared_ptr<IRoomClientObserver> observer)
        {
            UniversalObservable<IRoomClientObserver>::removeObserver(observer);
        }

        void join(const std::string &hostname, uint16_t port, const std::string &roomId, const std::string &displayName, std::shared_ptr<Options> options)
        {
            if (hostname.empty())
            {
        ...
        std::shared_ptr<IMediaController> getMediaController()
        {
        ...
        void getRouterRtpCapabilities()
        {
            ...
            _mediasoupApi->getRouterRtpCapabilities([wself = weak_from_this()](int32_t errorCode, const std::string &errorInfo, std::shared_ptr<signaling::GetRouterRtpCapabilitiesResponse> response)
                                                    ...
                                                    TMgr->thread("mediasoup-client")->PostTask(RTC_FROM_HERE, [wself, response]()
                                                                                               {
            auto self = wself.lock();
            if (!self) {
                DLOG("RoomClient is null");
        ...
        TMgr->thread("mediasoup-client")->PostTask(RTC_FROM_HERE,[wself, producing, consuming, response](){
            auto self = wself.lock();
            if (!self) {
                DLOG("RoomClient is null");
                return;
        ...
        std::future<void> OnConnect(mediasoupclient::Transport *transport, const nlohmann::json &dtlsParameters)
        {
        ...
        void onOpened()
        {
            TMgr->thread("mediasoup-client")->PostTask(RTC_FROM_HERE, [wself = weak_from_this()]()
                                                       {
        auto self = wself.lock();
        if (!self) {
            DLOG("RoomClient is null");
    ...
    protected:
        // Request from SFU
        void onNewConsumer(std::shared_ptr<signaling::NewConsumerRequest> request)
        {
        }

        void onNewDataConsumer(std::shared_ptr<signaling::NewDataConsumerRequest> request)
        {
        }

        // Notification from SFU
        void onProducerScore(std::shared_ptr<signaling::ProducerScoreNotification> notification)
        {
        }

        void onConsumerScore(std::shared_ptr<signaling::ConsumerScoreNotification> notification)
        {
        }

        void onNewPeer(std::shared_ptr<signaling::NewPeerNotification> notification)
        {
        }

        void onPeerClosed(std::shared_ptr<signaling::PeerClosedNotification> notification)
        {
        }

        void onPeerDisplayNameChanged(std::shared_ptr<signaling::PeerDisplayNameChangedNotification> notification)
        {
        }

        void onConsumerPaused(std::shared_ptr<signaling::ConsumerPausedNotification> notification)
        {
        }

        void onConsumerResumed(std::shared_ptr<signaling::ConsumerResumedNotification> notification)
        {
        }

        void onConsumerClosed(std::shared_ptr<signaling::ConsumerClosedNotification> notification)
        {
        }

        void onConsumerLayersChanged(std::shared_ptr<signaling::ConsumerLayersChangedNotification> notification)
        {
        }

        void onDataConsumerClosed(std::shared_ptr<signaling::DataConsumerClosedNotification> notification)
        {
        }

        void onDownlinkBwe(std::shared_ptr<signaling::DownlinkBweNotification> notification)
        {
        }

        void onActiveSpeaker(std::shared_ptr<signaling::ActiveSpeakerNotification> notification)
        {
        }
    ...
    RoomClient::RoomClient(std::weak_ptr<IComponentFactory> wcf)
        : _roomclientImpl(std::make_shared<RoomClientImpl>(wcf))
    {
    }
    ...
    void RoomClient::CleanupLibMediasoup()
    {
    ...
    {
        _roomclientImpl->join(hostname, port, roomId, displayName, options);
    }

    void RoomClient::leave()
    {
        _roomclientImpl->leave();
    }

    std::shared_ptr<IMediaController> RoomClient::getMediaController()
    {
        return _roomclientImpl->getMediaController();
    }

    std::shared_ptr<IParticipantController> RoomClient::getParticipantController()
    {
        return _roomclientImpl->getParticipantController();
    }
