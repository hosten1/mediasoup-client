...
public:
  RoomClientImpl(std::weak_ptr<IComponentFactory> wcf)
      : _wcf(wcf), _inited(false), _destroy(false) {
    // 初始化逻辑
  }
  ...
  void init() {
    ...
    if (!_participantController.lock()) {
      ...
      _participantControllerProxy = IParticipantControllerProxy::create(
          participantController, "mediasoup-client");
  ...
  void addObserver(std::shared_ptr<IRoomClientObserver> observer) {
    UniversalObservable<IRoomClientObserver>::addWeakObserver(
        observer, "mediasoup-client");
  }
  ...
  void join(const std::string &hostname, uint16_t port,
    ...
    if (_signalingClient) {
      ...
      UniversalObservable<IRoomClientObserver>::notifyObservers(
          [](const auto &observer) {
  ...
  void getRouterRtpCapabilities() {
    ...
    _mediasoupApi->getRouterRtpCapabilities(
        [wself = weak_from_this()](
          ...
          if (errorCode != 0) {
            DLOG("getRouterRtpCapabilities failed, error code: {}, error info: "
                 "{}",
                 errorCode, errorInfo);
            return;
          }
          ...
          TMgr->thread("mediasoup-client")
              ->PostTask(RTC_FROM_HERE, [wself, response]() {
                auto self = wself.lock();
                if (!self) {
                  DLOG("RoomClient is null");
                  return;
  ...
  void requestCreateTransport(bool forceTcp, bool producing, bool consuming) {
    ...
    _mediasoupApi->createWebRtcTransport(
        request, [wself = weak_from_this(), consuming, producing](
                     int32_t errorCode, const std::string &errorInfo,
          ...
          TMgr->thread("mediasoup-client")
              ->PostTask(RTC_FROM_HERE,
                         [wself, producing, consuming, response]() {
                           auto self = wself.lock();
                           if (!self) {
                             DLOG("RoomClient is null");
                             return;
  ...
  void joinImpl() {
    ...
    _mediasoupApi->join(
        request, [wself = weak_from_this()](
          ...
          self->UniversalObservable<IRoomClientObserver>::notifyObservers(
              [](const auto &observer) {
  ...
  std::future<std::string> OnProduce(mediasoupclient::SendTransport *transport,
    ...
    std::future<std::string> fut =
        std::async(std::launch::async, [wself = weak_from_this(), transport,
  ...
  void _onConnect(mediasoupclient::Transport *transport,
    ...
    std::string err;
    auto dtlsp = fromJsonString<
        signaling::ConnectWebRtcTransportRequest::DTLSParameters>(json, err);
    if (!err.empty()) {
      DLOG("parse response failed: {}", err);
      return;
    }
    ...
    _mediasoupApi->connectWebRtcTransport(
        request, [wself = weak_from_this(), &promise](
          ...
          if (errorCode != 0) {
            DLOG(
                "connectWebRtcTransport failed, error code: {}, error info: {}",
                errorCode, errorInfo);
            return;
          }
          if (!response || !response->ok) {
  ...
  std::string _onProduceData(mediasoupclient::SendTransport *transport,
    ...
    std::string err;
    auto stcpsp =
        fromJsonString<signaling::ProduceDataRequest::SCTPStreamParameters>(
            json, err);
    if (!err.empty()) {
      DLOG("parse response failed: {}", err);
      return "";
    ...
    _mediasoupApi->produceData(
        request, [wself = weak_from_this(), &promise](
          ...
          if (errorCode != 0) {
            DLOG("produceData failed, error code: {}, error info: {}",
                 errorCode, errorInfo);
            return;
          }
          if (!response || !response->ok) {
  ...
  void configWebrtc() {
    ...
    if (!this->_networkThread->Start() || !this->_signalingThread->Start() ||
        !this->_workerThread->Start()) {
      ELOG("thread start errored");
    }

    this->_peerConnectionFactory = webrtc::CreatePeerConnectionFactory(
        this->_networkThread.get(), this->_workerThread.get(),
        this->_signalingThread.get(), nullptr /*default_adm*/,
    ...
    _peerConnectionOptions.reset(
        new mediasoupclient::PeerConnection::Options());
...
void RoomClient::CleanupLibMediasoup() { mediasoupclient::Cleanup(); }

void RoomClient::init() { _roomclientImpl->init(); }

void RoomClient::destroy() { _roomclientImpl->destroy(); }
...
void RoomClient::leave() { _roomclientImpl->leave(); }
