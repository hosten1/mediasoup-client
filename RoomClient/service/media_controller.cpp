...
void MediaController::addObserver(
    std::shared_ptr<IMediaControllerObserver> observer) {
  UniversalObservable<IMediaControllerObserver>::addWeakObserver(
  ...
  if (enabled) {
    ...
    if (_capturerSource) {
      ...
      rtc::scoped_refptr<webrtc::VideoTrackInterface> track =
          _peerConnectionFactory->CreateVideoTrack("camera-track",
                                                   _capturerSource);
      ...
      UniversalObservable<IMediaControllerObserver>::notifyObservers(
          [wself = weak_from_this()](const auto &observer) {
            auto self = wself.lock();
            if (!self) {
              DLOG("RoomClient is null");
              return;
            }
            observer->onCreateVideoTrack(self->_camProducer->GetId(),
                                         self->_camProducer->GetTrack());
          });
    }
  } else {
    if (!_mediasoupApi) {
...
void MediaController::createNewConsumer(
  ...
  UniversalObservable<IMediaControllerObserver>::notifyObservers(
      [wself = weak_from_this(), ptr](const auto &observer) {
        auto self = wself.lock();
        if (!self) {
          DLOG("RoomClient is null");
          return;
...
void MediaController::onConsumerClosed(
  ...
  UniversalObservable<IMediaControllerObserver>::notifyObservers(
      [wself = weak_from_this(), notification](const auto &observer) {
        auto self = wself.lock();
        if (!self) {
          DLOG("RoomClient is null");
          return;
        ...
        for (const auto &consumer : self->_consumerMap) {
          if (consumer.second->GetId() ==
              ...
              consumer.second->GetKind() == "video") {
            observer->onRemoveVideoTrack(consumer.second->GetId(),
                                         consumer.second->GetTrack());
...
void MediaController::onActiveSpeaker(
