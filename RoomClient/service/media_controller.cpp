...
namespace vi
{
    ...
    MediaController::MediaController(std::shared_ptr<IMediasoupApi> &mediasoupApi,
        ...
        : _mediasoupApi(mediasoupApi), _sendTransport(sendTransport), _recvTransport(recvTransport), _peerConnectionFactory(pcf), _options(options)
    {
    }
    ...
    {
        TMgr->thread("mediasoup-client")->PostTask(RTC_FROM_HERE, [wself = weak_from_this(), request]()
                                                   {
        auto self = wself.lock();
        if (!self) {
            DLOG("RoomClient is null");
